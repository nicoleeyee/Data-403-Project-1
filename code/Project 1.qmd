...
p5 <- model_data |>
  ...
  pivot_longer(cols = c("pct_hispanic", "pct_black", "pct_api", "pct_aian", "pct_white"),
               names_to = "ethnicity",
               values_to = "ethnicity_count",
               names_prefix = "pct_") |>
  group_by(county_1, ethnicity) |>
  summarise(pct = mean(ethnicity_count),
            rev = mean(ratio)) |>
...
p9 |>
  ggplot(aes(x=pct, y=log(rev), color = ethnicity)) +
  geom_jitter() +
  scale_color_manual(name="Ethnicity", values = c("#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "darkgreen")) +
  facet_wrap(~ethnicity) +
  theme_minimal() +
  labs(x="% of Population",
       y = "Alcohol Sales ($)",
